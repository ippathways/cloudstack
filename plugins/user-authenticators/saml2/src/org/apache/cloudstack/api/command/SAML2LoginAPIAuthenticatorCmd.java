            ...
            if (!params.containsKey(SAMLPluginConstants.SAML_RESPONSE) && !params.containsKey("SAMLart")) {
                ...
                try {
                    if (_apiServer.verifyUser(userAccount.getId())) {
                        ...
                        s_logger.debug("Writing jsessionID to token table: " + (((String)session.getAttribute(SAMLPluginConstants.SAML_NAMEID) == null) ? "NULL" : (String)session.getAttribute(SAMLPluginConstants.SAML_NAMEID)));
