        ...
        try {
            if (!params.containsKey(SAMLPluginConstants.SAML_RESPONSE) && !params.containsKey("SAMLart")) {
                ...
                if (params.containsKey(ApiConstants.REDIRECT_ON_ERROR)) {
                    try {
                        redirectOnError = ((Boolean[])params.get(ApiConstants.REDIRECT_ON_ERROR))[0];
                    } catch (Exception e) {
                        s_logger.error("SAML " + ApiConstants.REDIRECT_ON_ERROR + " is not true or false, defaulting to false. (" + ((String[])params.get(ApiConstants.REDIRECT_ON_ERROR))[0] + ") " + e.getMessage());
                    }
                }
                redirectOnError = getRedirectOnError();
                if (redirectOnError == null) {
                    s_logger.debug("SAML RedirectOnError is null");
                } else {
                    s_logger.debug("SAML RedirectOnError: " + redirectOnError.toString());
                }
                ...
                String redirectUrl = SAMLUtils.buildAuthnRequestUrl(authnId, spMetadata, idpMetadata, SAML2AuthManager.SAMLSignatureAlgorithm.value(), req, redirectOnError);
            ...
            if (redirectOnError) {
                try {
                    resp.addCookie(new Cookie(SAMLPluginConstants.SAML_LOGIN_MSG_COOKIE, SAMLUtils.getErrorTextFromXml(e.getDescription())));
                    SAMLUtils.redirectToSAMLCloudStackRedirectionUrl(resp, req);
                } catch (IOException e2) {
                    auditTrailSb.append("SAML HTTP redirection on error failed:");
                    auditTrailSb.append(e2.getMessage());
                } catch (Exception e3) {
                    s_logger.error("SAML Unable to redirect on login error: " + e3.getMessage());
                }
            } else {
                throw(e);
            }
