    ...
    public String authenticate(String command, Map<String, Object[]> params, HttpSession session, InetAddress remoteAddress, String responseType, StringBuilder auditTrailSb, final HttpServletRequest req, final HttpServletResponse resp) throws ServerApiException {
        ...
        if (SAML2AuthManager.SAMLIsIdentityProviderSloEnabled.value() && params != null && params.containsKey("SAMLRequest")) {
            ...
            if (processedSAMLRequest != null) {
                ...
                if (session == null || !samlSessionIndex.equals(targetSessionIndex)) {
                    final SAMLTokenVO token = _samlAuthManager.getTokenBySessionIndex(targetSessionIndex);
                    if (token != null) {
                        try {
                            s_logger.debug("SAML redirecting to user's Slo URL");
                        ...
                        s_logger.debug("SAML unable to find a matching session index, sending failure to IdP" + targetSessionIndex);
                        try {
                            resp.sendRedirect(SAMLUtils.buildLogoutResponseUrl(processedSAMLRequest.getID(), spMetadata, idpMetadata, StatusCode.REQUESTER_URI, "Session Index not found", SAML2AuthManager.SAMLSignatureAlgorithm.value()));
                        } catch (IOException ignored) {
                    ...
                    try {
                            s_logger.debug("SAML IdP initiated Slo successful, sending success to IdP");
