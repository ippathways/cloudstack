        ...
        if (SAML2AuthManager.SAMLIsIdentityProviderSloEnabled.value() && params != null && params.containsKey("SAMLRequest")) {
            ...
            if (session == null) {
                ...
                if (nameId != null) {
                    ...
                    if (token != null) {
                        s_logger.debug("SAML token found! " + token.getId());
                        try {
                            /*resp.sendRedirect(token.getJsessionId());*/
                        ...
                        s_logger.debug("SAML token not found for " + nameId);
                    }
                } else {
                    s_logger.debug("SAML unable to find nameId in Slo");
                }
            } else {
